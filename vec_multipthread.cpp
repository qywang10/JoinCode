...
double *VECTORJOIN_multip(int32_t *R_key, int32_t *R_payload, int32_t *S_key, int32_t *S_payload, uint64_t R_num_tuples, uint64_t S_num_tuples,int num_nthreads)
{
    timeval start1, end1;
    timeval buildp_start,buildp_end;
    clock_t start, end;
    // int nthreads = sysconf(_SC_NPROCESSORS_ONLN);
    int nthreads = num_nthreads;
    int32_t numR, numS, numRthr, numSthr;
    int i, rv, j;
    float sum_p_create;
    cpu_set_t set;
...
void multi_noband_nonuma(param_join_t cmd_params,string cpumodel,string numastat,int num_thread){
    string metricstat;
    char filename[40];
    sprintf(filename,"./test_vecjoin_cpu_multi_xlsx_%d.txt",num_thread);
    FILE *fpans=fopen(filename,"w");    // cpu连接测试xlsx格式
    fprintf(fpans, "%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "R_size", "s_size", "CPU", "NUMA", "Metric", "Values","multi_threads");
    ...
    for (int i = 0; i <= 30; i++)
        {
            ...
            *(S->num_tuples) = cmd_params.s_size;
            ...
            for (int j = 0; j < 5; j++)
            {
                double *run_time_tmp = VECTORJOIN_multip(R->key, R->payload, S->key, S->payload, cmd_params.r_size, S->num_tuples[0],num_thread);
            ...
            fprintf(fpans,"%d\t%lld\t%lld\t%s\t%s\t%s\t%lf\n",nth,cmd_params.r_size, cmd_params.s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), min_run_time*1000);
            metricstat="Throughput(GT/s)";
            fprintf(fpans,"%d\t%d\t%d\t%s\t%s\t%s\t%lf\n",nth,cmd_params.r_size, cmd_params.s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), 1 / min_run_time);
            metricstat="Cycles/Tuple";
            fprintf(fpans,"%d\t%lld\t%lld\t%s\t%s\t%s\t%lf\n",nth,cmd_params.r_size, cmd_params.s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), cmd_params.s_size / min_run_clock);
...
void multi_noband_numa(int numa_regions,param_join_t cmd_params,string cpumodel,string numastat,int num_thread){
    string metricstat;
    char filename[48];
    sprintf(filename,"./test_vecjoin_cpu_multi_numa_xlsx_%d.txt",num_thread);
    FILE *fpans=fopen(filename,"w");    // cpu连接测试xlsx格式
    fprintf(fpans, "%s\t%s\t%s\t%s\t%s\t%s\t%s\n", "R_size", "s_size", "CPU", "NUMA", "Metric", "Values","multi_threads");
    ...
    for (int i = 0; i <= 30; i++)
    {
        ...
        *(R->num_tuples) = cmd_params.r_size;
        ...
        int num_size_per_numa = cmd_params.s_size / numa_regions;
        for (int j = 0; j < numa_regions; j++)
            num_numa[j] = (j == numa_regions - 1) ? cmd_params.s_size - num_size_per_numa * (numa_regions - 1) : num_size_per_numa;
        ...
        metricstat="Times(ms)";
        fprintf(fpans,"%d\t%d\t%s\t%s\t%s\t%lf\n",cmd_params.r_size, cmd_params.s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), min_run_time*1000);
        metricstat="Throughput(GT/s)";
        fprintf(fpans,"%d\t%d\t%s\t%s\t%s\t%lf\n",cmd_params.r_size, cmd_params.s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), 1 / min_run_time);
        metricstat="Cycles/Tuple";
        fprintf(fpans,"%d\t%d\t%s\t%s\t%s\t%lf\n",cmd_params.r_size, cmd_params.s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), cmd_params.s_size / min_run_clock);
...
int main(int argc, char **argv)
{
    ...
    {
        printf("\nIN NO_BANDWIDTH AND NO_NUMA\n\n");

        int a=20;
        numastat="NUMA-oblivious";    // NUMA-conscious
        
        multi_noband_nonuma(cmd_params,cpumodel,numastat,a);
    }
    if (cmd_params.test_bandwidth && !cmd_params.numa_partition  && !cmd_params.cachetest)
    {
        FILE *fpans = fopen("./test_vecjoin_cpu_xlsx.txt", "w");    // cpu连接测试xlsx格式
    ...
    if (!cmd_params.test_bandwidth && cmd_params.numa_partition  && !cmd_params.cachetest)
    {
        printf("\nIN NO_BANDWIDTH AND NUMA\n\n");

        numastat="NUMA-conscious";    // NUMA-conscious
        multi_noband_numa(numa_regions,cmd_params,cpumodel,numastat,12);
    }
    if (cmd_params.test_bandwidth && cmd_params.numa_partition  && !cmd_params.cachetest)
    {
        FILE *fpans = fopen("./test_vecjoin_cpu_xlsx.txt", "w");    // cpu连接测试xlsx格式
