...
int main(int argc, char **argv)
{
    ...
    if (!cmd_params.test_bandwidth && !cmd_params.numa_partition && !cmd_params.cachetest)
    {
        ...
        for (int i = 0; i <= 30; i++)
        {
            ...
            *(S->num_tuples) = cmd_params.s_size;
            ...
            metricstat="Times(ms)";
            fprintf(fpans,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",cmd_params.r_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), min_run_time*1000);
            metricstat="Throughput(GT/s)";
            fprintf(fpans,"%d\t%d\t%s\t%s\t%s\t%lf\n",cmd_params.r_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), 1 / min_run_time);
            metricstat="Cycles/Tuple";
            fprintf(fpans,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",cmd_params.r_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), s_size / min_run_clock);
    ...
    if(cmd_params.cachetest){
        ...
        FILE *fp = fopen("./test_vecjoin_cpu_cache_test.txt", "w");
        fprintf(fp, "%s\t%s\t%s\t%s\t%s\t%s\n", "R_size", "s_size", "time(s)", "tuples/s", "CPU_Clock", "tuple/CPU Clock");

        FILE *fpcache = fopen("./test_vecjoin_cpu_cache_xlsx.txt", "w");    // cache_size_test 连接测试xlsx格式
        fprintf(fpcache, "%s\t%s\t%s\t%s\t%s\t%s\n", "R_size", "s_size", "CPU", "NUMA", "Metric", "Values");

        // numastat="NUMA-oblivious";    // NUMA-conscious
        /*---------- get L1 Cache test ----------*/
        if(cmd_params.cachetest==1){
            cout<<"L1 cache test start\n";
            for (int i = 1; i <= 15; i++)    // 10:100%
            {
                ...
                *(S->num_tuples) = cmd_params.s_size;
                ...
                // 下面可要
                metricstat="Times(ms)";
                fprintf(fpcache,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",r_l1_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), min_run_time*1000);
                metricstat="Throughput(GT/s)";
                fprintf(fpcache,"%d\t%d\t%s\t%s\t%s\t%lf\n",r_l1_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), 1 / min_run_time);
                metricstat="Cycles/Tuple";
                fprintf(fpcache,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",r_l1_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), s_size / min_run_clock);
                ...
                *   time 单位为s
                */
                fprintf(fp, "%d\t%lld\t%lf\t%lf\t%lf\t%lf\n", r_l1_size, cmd_params.s_size, min_run_time, 1 / min_run_time, min_run_clock, min_run_clock / cmd_params.s_size);
                //R_size	s_size	time(s)	tuples/s	CPU Clock	tuple/CPU Clock
                if (i == 0)
                    i += 4;
            }
            ...
            for (int i = 1; i <= 15; i++)    // 10:100%
            {
                ...
                *(S->num_tuples) = cmd_params.s_size;
                ...
                // 下面可要
                metricstat="Times(ms)";
                fprintf(fpcache,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",r_l2_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), min_run_time*1000);
                metricstat="Throughput(GT/s)";
                fprintf(fpcache,"%d\t%d\t%s\t%s\t%s\t%lf\n",r_l2_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), 1 / min_run_time);
                metricstat="Cycles/Tuple";
                fprintf(fpcache,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",r_l2_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), s_size / min_run_clock);
                ...
                *   time 单位为s
                */
                fprintf(fp, "%d\t%lld\t%lf\t%lf\t%lf\t%lf\n", r_l2_size, cmd_params.s_size, min_run_time, 1 / min_run_time, min_run_clock, min_run_clock / cmd_params.s_size);
                //R_size	s_size	time(s)	tuples/s	CPU Clock	tuple/CPU Clock
                if (i == 0)
                    i += 4;
            }
            ...
            for (int i = 1; i <= 15; i++)    // 10:100%
            {
                ...
                *(S->num_tuples) = cmd_params.s_size;
                ...
                // 下面可要
                metricstat="Times(ms)";
                fprintf(fpcache,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",r_l3_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), min_run_time*1000);
                metricstat="Throughput(GT/s)";
                fprintf(fpcache,"%d\t%d\t%s\t%s\t%s\t%lf\n",r_l3_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), 1 / min_run_time);
                metricstat="Cycles/Tuple";
                fprintf(fpcache,"%lld\t%lld\t%s\t%s\t%s\t%lf\n",r_l3_size, s_size, cpumodel.c_str(), numastat.c_str(), metricstat.c_str(), s_size / min_run_clock);
                ...
                *   time 单位为s
                */
                fprintf(fp, "%d\t%lld\t%lf\t%lf\t%lf\t%lf\n", r_l3_size, cmd_params.s_size, min_run_time, 1 / min_run_time, min_run_clock, min_run_clock / cmd_params.s_size);
                //R_size	s_size	time(s)	tuples/s	CPU Clock	tuple/CPU Clock
                if (i == 0)
                    i += 4;
            }
